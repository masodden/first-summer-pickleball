import type { TranslationKey } from './ru.js';

export const en: Record<TranslationKey, string> = {
  'app.name': 'PICKLEBALL Events',
  'app.shortName': 'PICKLEBALL Events',
  'app.tagline': 'Local pickleball tournaments',

  'common.save': 'Save',
  'common.cancel': 'Cancel',
  'common.close': 'Close',
  'common.create': 'Create',
  'common.delete': 'Delete',
  'common.edit': 'Edit',
  'common.add': 'Add',
  'common.back': 'Back',
  'common.next': 'Next',
  'common.retry': 'Retry',
  'common.confirm': 'Confirm',
  'common.search': 'Search',
  'common.loading': 'Loading',
  'common.empty': 'Nothing here yet',
  'common.yes': 'Yes',
  'common.no': 'No',
  'common.of': 'of',
  'common.optional': 'optional',
  'common.copy': 'Copy',
  'common.copied': 'Copied',
  'common.share': 'Share',
  'common.apply': 'Apply',
  'common.reset': 'Reset',
  'common.notSet': 'not set',
  'common.actions': 'Actions',
  'common.more': 'More',
  'common.date': 'Date',
  'common.time': 'Time',
  'common.online': 'Connected',
  'common.offline': 'No connection',
  'common.reconnecting': 'Reconnecting',
  'common.pendingActions': 'Pending actions: {{count}}',

  'nav.tournaments': 'Tournaments',
  'nav.trainings': 'Open Play',
  'nav.players': 'Players',
  'nav.profile': 'Profile',
  'nav.settings': 'Settings',
  'nav.admin': 'Admin',
  'nav.about': 'The game',

  'auth.loginTelegram': 'Sign in with Telegram',
  'auth.notInTelegram': 'Opened outside Telegram',
  'auth.notInTelegramHint':
    'You can browse tournaments and standings without signing in. To enter tournaments, open the app in Telegram.',
  'auth.continueAsSpectator': 'Continue as spectator',
  'auth.spectatorMode': 'Spectator mode',
  'auth.signOut': 'Sign out',
  'auth.signInAgain': 'Sign in again',
  'auth.signedOutHint':
    'You signed out. Tap “Sign in again” to restore your account. If that fails, close the Mini App and reopen the bot.',
  'auth.bootstrapTitle': 'Administrator verification',
  'auth.bootstrapHint':
    'This DUPR ID is reserved for a club administrator. Enter the BOOTSTRAP_ADMIN_CODE from the server .env file.',
  'auth.bootstrapCode': 'Administrator code',

  'claim.title': 'Link your DUPR',
  'claim.hint':
    'Find yourself by name or enter your DUPR ID. An organiser will confirm the link at tournament check-in.',
  'claim.changeTitle': 'Change DUPR ID',
  'claim.changeHint':
    'Enter a new DUPR ID. The request goes to an organiser for approval. Admin IDs PZQZKM and P5ML0M need the code from .env.',
  'claim.changeSubmit': 'Change DUPR',
  'claim.duprId': 'DUPR ID',
  'claim.duprIdHint':
    'Six characters, for example A1B2C3. You can see it in your DUPR app profile.',
  'claim.searchPlaceholder': 'Name or DUPR ID',
  'claim.notFound': "Can't find yourself? Create a profile manually",
  'claim.submit': 'Link',
  'claim.pending': 'Request sent, waiting for organiser approval',
  'claim.approved': 'DUPR linked',
  'claim.rejected': 'Request rejected. Please contact an organiser',
  'claim.alreadyClaimed': 'This DUPR ID is already linked to another account',
  'claim.requests': 'Link requests',
  'claim.approve': 'Approve',
  'claim.reject': 'Reject',
  'claim.noRequests': 'No new requests',
  'claim.inviteTitle': 'Invite a player',
  'claim.inviteHint':
    'Send this link to the player. Opening it links their Telegram to the profile with no manual approval.',
  'claim.inviteCreate': 'Create invite link',
  'claim.inviteCopy': 'Copy link',
  'claim.inviteCopied': 'Link copied',
  'claim.inviteApplied': 'Telegram linked to this profile',
  'claim.inviteExpires': 'Link valid until {{date}}',

  'role.admin': 'Administrator',
  'role.moderator': 'Moderator',
  'role.organizer': 'Organizer',
  'role.user': 'Player',
  'role.spectator': 'Spectator',
  'role.change': 'Change role',
  'role.changed': 'Role updated',

  'tournament.list': 'Tournaments',
  'tournament.create': 'Create tournament',
  'tournament.createTitle': 'New tournament',
  'tournament.edit': 'Edit tournament',
  'tournament.empty': 'No tournaments yet',
  'tournament.emptyHint': 'Create your first tournament to get started',
  'tournament.emptyFinished': 'No finished tournaments yet',
  'tournament.emptyArchive': 'Archive is empty',
  'tournament.title': 'Title',
  'tournament.category': 'Category',
  'tournament.categoryHint': 'For example advanced or intermediate',
  'tournament.format': 'Format',
  'tournament.startsAt': 'Date and time',
  'tournament.courts': 'Number of courts',
  'tournament.courtNames': 'Court names',
  'tournament.courtNamesHint':
    'How the courts are labelled at the venue: 4, 5, 6 or “Centre court”. Empty ones keep their numbers. The first court in the list is the top one: in mexicano the leaders play there.',
  'tournament.courtSlot': 'Court #{{number}}',
  'tournament.maxPlayers': 'Number of players',
  'tournament.pointsToWin': 'Maximum score',
  'tournament.pointsToWinGroup': 'Group score',
  'tournament.pointsToWinPlayoff': 'Playoff score',
  'tournament.matchDuration': 'Match duration, minutes',
  'tournament.matchDurationHint': 'Leave empty to play to the score only',
  'tournament.rounds': 'Number of games',
  'tournament.roundsHint': 'Leave empty to play until the organiser stops the tournament',
  'tournament.roundsInfinite': 'Until stopped',
  'tournament.tieRule': 'When the timer ends on a tie',
  'tournament.standingsSort': 'Winner determination rule',
  'tournament.winnerRule.pointsDiff': 'Total points + differential',
  'tournament.winnerRule.pointsWins': 'Total points + wins',
  'tournament.winnerRule.winsPoints': 'Wins + points',
  'tournament.ratingBalance': 'Balance teams by DUPR',
  'tournament.ratingBalanceHint': 'Even out rating sums when building and reshuffling teams',
  'tournament.entryFee': 'Entry fee',
  'tournament.description': 'Tournament description',
  'tournament.formatDescription': 'Format description',
  'tournament.venue': 'Venue',
  'tournament.venueName': 'Venue name',
  'tournament.venueAddress': 'Address',
  'tournament.venueMapUrl': 'Map link',
  'tournament.openMap': 'Open in maps',
  'tournament.savedVenues': 'Saved venues',
  'tournament.created': 'Tournament created',
  'tournament.updated': 'Changes saved',
  'tournament.deleted': 'Tournament deleted',
  'tournament.deleteConfirm': 'Delete tournament "{{title}}"? This cannot be undone.',
  'tournament.closeRegistration': 'Close registration',
  'tournament.reopenRegistration': 'To registration',
  'tournament.start': 'Build tournament and start',
  'tournament.startShort': 'Start tournament',
  'tournament.startHint': 'All participants are confirmed, games can be built',
  'tournament.startConfirm':
    'Build games for {{count}} participants? They will get a Telegram notification.',
  'tournament.startConfirmPairs':
    'Build games for {{count}} pairs? They will get a Telegram notification.',
  'tournament.pairsCount': '{{count}} pairs',
  'tournament.pairsCountOf': '{{count}} of {{max}} pairs',
  'tournament.registrationClosedHint':
    'Registration is closed: the organizer builds the roster. Open registration if players should sign up themselves.',
  'tournament.unstart': 'Back to registration',
  'tournament.unstartConfirm':
    'Roll the tournament back to “Registration closed”? The schedule will be deleted — you can replace a player and build games again.',
  'tournament.unstarted': 'Tournament rolled back to registration',
  'tournament.finish': 'Finish tournament',
  'tournament.finishShort': 'Finish',
  'tournament.finishConfirm':
    'Finish the tournament? Scores will lock and medals will appear. If you made a mistake, you can put it back in play.',
  'tournament.finished': 'Tournament finished',
  'tournament.reopen': 'Back to play',
  'tournament.reopenConfirm':
    'The tournament will be “In progress” again: you can edit scores. Medals will disappear until you finish it once more.',
  'tournament.reopened': 'Tournament is live again — scores can be edited',
  'tournament.archive': 'Archive',
  'tournament.unarchive': 'Unarchive',
  'tournament.archived': 'Tournament archived',
  'tournament.unarchived': 'Tournament restored from archive',
  'tournament.exportCsv': 'CSV for DUPR',
  'tournament.publicLink': 'Public standings link',
  'tournament.boardLinkShort': 'Live board',
  'tournament.appLink': 'Link to tournament in the bot',
  'tournament.appLinkShort': 'Link',
  'tournament.appLinkCopied': 'Link copied — share it with players in chat',
  'tournament.appLinkMissingBot':
    'TELEGRAM_BOT_USERNAME is not set on the server — cannot build an in-bot link',
  'tournament.announceRegistration': 'Broadcast',
  'tournament.announceRegistrationSend': 'Send',
  'tournament.announceRegistrationConfirm':
    'This will go to {{count}} players with Telegram. People already on this tournament roster will not get it. Notifications can be turned off in Settings.',
  'tournament.announceRegistrationSent': 'Broadcast sent: {{count}}',
  'tournament.announceRegistrationEmpty':
    'Nobody to notify: no Telegram players, or everyone is already on the roster',
  'tournament.switchTo': 'Go to tournament',
  'tournament.parallel': 'Parallel',
  'tournament.info': 'Information',
  'tournament.participantsCount': '{{count}} of {{max}}',

  'status.registration': 'Registration open',
  'status.registration_closed': 'Registration closed',
  'status.running': 'In progress',
  'status.finished': 'Finished',
  'status.archived': 'Archived',
  'status.activeFilter': 'Active',
  'status.finishedFilter': 'Finished',
  'status.archiveFilter': 'Archive',

  'format.americano': 'Americano',
  'format.mexicano': 'Mexicano',
  'format.fixed_pairs': 'Fixed pairs',
  'format.americano.description': 'Everyone partners with everyone: partners rotate each round.',
  'format.mexicano.description':
    'Teams for each round are built from the current standings: leaders move up to the top courts, others move down.',
  'format.fixed_pairs.description':
    'Pairs are locked in advance. Groups play round-robin, then quarters, semis, a final and placement matches.',

  'tie.draw': 'Allow a draw',
  'tie.golden_point': 'Play out a golden point',

  'participant.list': 'Participants',
  'participant.pairs': 'Pairs',
  'participant.empty': 'No one has signed up yet',
  'participant.add': 'Add player',
  'participant.addFromBase': 'Pick from database',
  'participant.addNew': 'Create profile',
  'participant.join': 'Enter tournament',
  'participant.leave': 'Withdraw',
  'participant.joined': 'You have entered the tournament',
  'participant.left': 'Entry withdrawn',
  'participant.added': 'Player added',
  'participant.removed': 'Player removed',
  'participant.remove': 'Remove from tournament',
  'participant.needDupr': 'Link your DUPR to enter tournaments',
  'participant.full': 'No spots left',
  'participant.searchPlaceholder': 'First name, last name or DUPR ID',
  'participant.notFound': 'No such player in the database',
  'participant.guestBadge': 'Guest',
  'participant.guestHint':
    'Prefer adding a DUPR ID now so rating and stats stay in sync. If they do not have one yet, leave it blank and add it later.',
  'participant.guestJoinHint':
    'Link your DUPR in Settings when you can — rating will follow. You can also do it later.',
  'participant.linkDuprLater': 'Link DUPR',
  'participant.selfAdded': 'Self-entered',
  'participant.unpaired': 'No partner',
  'partner.link': 'Link pair',
  'partner.unlink': 'Unlink',
  'partner.confirmJoin': 'Enter the tournament with {{name}}?',
  'partner.confirmLink': 'Pair with {{name}}?',
  'partner.confirmUnlink': 'Disband this pair?',
  'partner.linked': 'Pair locked in',
  'partner.unlinked': 'Pair unlinked',
  'partner.locked': 'Pair locked by payment',
  'partner.pick': 'Choose partner',
  'partner.needPairs': 'Every player needs a partner before you can start',
  'partner.rosterMixed': '{{pairs}} · {{unpaired}} unpaired',
  'bracket.title': 'Draw',
  'bracket.intro':
    'Groups play a round-robin first. Then quarters, semis, a final and matches for places 5–12.',
  'bracket.groups': 'Number of groups',
  'bracket.preset12': 'Preset: 12 pairs, 2 groups',
  'bracket.preset6': 'Preset: 6 pairs, 1 group',
  'bracket.groupSection': 'Group stage',
  'bracket.groupWinsToTake': 'Games to win',
  'bracket.groupWinsHint': '1 — a single game, 2 — first to two wins',
  'bracket.groupPointsToWin': 'Group game to',
  'bracket.groupPointsHint': 'Points needed to win a group game',
  'bracket.winsToTake': 'Wins to take the match',
  'bracket.stageWinsHint': '1 — a single set, 2 — first to two wins. Semis and the final can differ.',
  'bracket.stagePointsHint': 'Points to win a set. Semis and the final are set separately.',
  'bracket.winByTwo': 'Win by 2 points',
  'bracket.winByTwoHint': '11–10 is not enough — it has to be 11–9 or 12–10',
  'bracket.playoffSection': 'After the group',
  'bracket.playoffHint':
    'Each card is a round after the group. Score and series length are per round: semis can be to 15, the final a series to 11.',
  'bracket.stagePlayoff': 'Playoff',
  'bracket.stageConsolation': 'Placement',
  'bracket.match': '{{name}} {{number}}',
  'bracket.vs': 'vs',
  'bracket.sideA': 'This pair',
  'bracket.sideB': 'Plays against',
  'bracket.pickSource': 'Choose',
  'bracket.groupPlaces': 'Group places',
  'bracket.previousResults': 'Earlier matches',
  'bracket.groupPlace': 'Place {{place}}, group {{group}}',
  'bracket.groupPlaceSingle': 'Place {{place}} in the group',
  'bracket.winnerOf': 'Winner: {{match}}',
  'bracket.loserOf': 'Loser: {{match}}',
  'bracket.removeMatch': 'Remove match',
  'bracket.removeStage': 'Remove stage',
  'bracket.addStage': 'Add round',
  'bracket.addStageHint': 'A later round: final, 3rd place, 5th place.',
  'bracket.addSlot': 'Add match in this round',
  'bracket.addSlotHint':
    'Another match in this round if they run in parallel. Games to win the match: the field above.',
  'bracket.kind': 'Type',
  'bracket.groupMatches': 'Round-robin cycles',
  'bracket.stageName': 'Name',
  'bracket.stagePointsToWin': 'Game to',
  'bracket.preview': 'How they play',
  'bracket.previewLine': '{{match}}: {{a}} — {{b}} ({{rules}})',
  'bracket.setsOne': '1 set',
  'bracket.setsTo': 'first to {{count}}',
  'bracket.gameRulesOne': '1 set to {{points}}',
  'bracket.gameRulesSeries': 'first to {{wins}}, set to {{points}}',
  'bracket.sameRulesMany': 'Quarters, semis and placements',
  'tournament.formatRules': 'How they play',
  'bracket.issue.emptySources': 'Every match needs both pairs chosen',
  'bracket.issue.duplicateSources': 'The same placing cannot appear in two matches',
  'bracket.issue.noFinal': 'Add a final — who becomes the champion',
  'bracket.issue.noThirdPlace': 'Add a 3rd-place match',
  'bracket.cannotStart': 'Finish the draw first: a final and a 3rd-place match',
  'match.startMatch': 'Start match',
  'match.playersBusy': 'These players are still on another court',
  'match.game': 'Game {{number}}',
  'match.series': 'Series {{a}}:{{b}}',
  'standings.pair': 'Pair',

  'waitlist.title': 'Waiting list',
  'waitlist.position': '{{position}} in queue',
  'waitlist.joined': 'You are on the waiting list',
  'waitlist.promoted': 'You have been moved into the main draw',
  'waitlist.promote': 'Move into the draw',
  'waitlist.replace': 'Replace',
  'waitlist.replaceTitle': 'Who should be replaced?',
  'waitlist.replaceHint':
    'Pick a player from the draw — they will be removed and {{name}} takes the spot.',
  'waitlist.replaced': 'Waitlisted player took a spot in the draw',
  'waitlist.replaceConfirm': 'Replace {{outgoing}} with {{incoming}}?',
  'waitlist.remove': 'Remove from waiting list',
  'waitlist.removed': 'Removed from waiting list',

  'checkin.title': 'Check-in',
  'checkin.hint':
    'Tick the players who showed up and paid. You can fix the rating right here without opening the profile.',
  'checkin.paid': 'Confirmed and paid',
  'checkin.confirmedOf': 'Confirmed {{confirmed}} of {{total}}',
  'checkin.allConfirmed': 'All participants confirmed',
  'checkin.notAllConfirmed': 'Some participants are not confirmed yet',

  'match.courtsTab': 'Games',
  'match.round': 'Round {{index}}',
  'match.roundOf': 'Round {{index}} of {{total}}',
  'match.court': 'Court {{number}}',
  'match.start': 'Start game',
  'match.startRound': 'Start round',
  'match.finishRound': 'Finish',
  'match.skipRound': 'Skip',
  'match.skipRoundConfirm':
    'Skip this round? You can come back to it later. The next round can start in the meantime.',
  'match.unskipRound': 'Restore round',
  'match.roundWaitingPrevious': 'Finish or skip the previous round first',
  'match.roundWaitingLive': 'Finish the round currently on court first',
  'match.roundSkipped': 'Round skipped',
  'match.roundNotStarted': 'The round has not started yet',
  'match.scoreAfterFinish': 'Enter the score when the court finishes',
  'match.pause': 'Pause',
  'match.resume': 'Resume',
  'match.finish': 'Finish game',
  'match.finishEarly': 'Finish early',
  'match.started': 'Game started',
  'match.paused': 'Paused',
  'match.finishedLabel': 'Game finished',
  'match.skippedLabel': 'Skipped',
  'match.timeUp': "Time's up",
  'match.timeUpHint': 'Finish the game manually once the rally is over',
  'match.waiting': 'Waiting to start',
  'match.sittingOut': 'Sitting out this round',
  'match.vs': 'vs',
  'match.enterScore': 'Enter score',
  'match.nextRound': 'Go to next round',
  'match.createNextRound': 'Create next round',
  'match.previousRound': 'Previous round',
  'match.reshuffle': 'Reshuffle players',
  'match.reshuffleConfirm': 'Reshuffle the teams? The schedule will be rebuilt.',
  'match.reshuffled': 'Teams reshuffled',
  'match.reshuffleLocked': 'Reshuffling is only possible before the first game starts',
  'match.generateSchedule': 'Build games',
  'match.noRounds': 'Games have not been built yet',
  'match.allRoundsDone': 'All planned games have been played',
  'match.roundNotScored': 'Enter the score for every game in the round',

  'score.title': 'Match score',
  'score.hint': 'Enter the final score. It can be edited until the tournament is finished.',
  'score.save': 'Save score',
  'score.saved': 'Score saved',
  'score.edit': 'Edit score',
  'score.tieNotAllowed': 'Draws are not allowed in this tournament',
  'score.needPoints': 'Games are played to {{points}}',
  'score.needWinByTwo': 'Must win by two points',
  'score.seriesIncomplete': 'Finish the series to a winner',
  'score.conflict': 'The score was already changed on another device',
  'score.conflictHint': 'Refresh the data and enter the score again',
  'score.useServerValue': 'Take the current score',
  'score.keepMine': 'Keep my score',

  'standings.title': 'Standings',
  'standings.results': 'Results',
  'standings.groupStage': 'Group stage',
  'standings.groupN': 'Group {{number}}',
  'standings.podium': 'Podium',
  'standings.tbd': '—',
  'standings.rank': '#',
  'standings.player': 'Player',
  'standings.played': 'Games',
  'standings.wins': 'Wins',
  'standings.losses': 'Losses',
  'standings.draws': 'Draws',
  'standings.points': 'Points',
  'standings.pointsAgainst': 'Conceded',
  'standings.diff': 'Difference',
  'standings.live': 'Live',
  'standings.empty': 'No games played yet',
  'standings.sortBy': 'Sort by',
  'standings.sortHint': 'Tap to sort; tap again to reverse',
  'standings.sortAsc': 'ascending',
  'standings.sortDesc': 'descending',
  'standings.medalGold': 'First place',
  'standings.medalSilver': 'Second place',
  'standings.medalBronze': 'Third place',

  'player.card': 'Player card',
  'player.profile': 'Profile',
  'player.firstName': 'First name',
  'player.lastName': 'Last name',
  'player.avatar': 'Avatar',
  'player.avatarUrl': 'Avatar link',
  'player.telegram': 'Telegram',
  'player.telegramHint': 'Username without @, e.g. ivan_pickle',
  'player.telegramLink': 'Link Telegram',
  'player.telegramLinked': 'Telegram linked',
  'player.telegramNoUsername': 'Telegram linked, but no @username',
  'player.telegramNoUsernameHint':
    'Without a public username you cannot open a Telegram chat. Ask the player to message the organiser.',
  'player.nudgeContact': 'Remind to message @{{contact}}',
  'player.nudgeContactSent': 'Reminder sent via the bot',
  'player.duprId': 'DUPR ID',
  'player.stats': 'Statistics',
  'player.statsTournaments': 'Tournaments',
  'player.statsMatches': 'Matches',
  'player.statsWins': 'Wins',
  'player.statsMedals': 'Medals',
  'player.noStats': 'No tournaments played yet',
  'player.directory': 'Player database',
  'player.createTitle': 'New player profile',
  'player.created': 'Player added to the database',
  'player.updated': 'Player details updated',
  'player.duplicateDupr': 'A player with this DUPR ID already exists',
  'player.mergeGuest': 'Set DUPR ID',
  'player.mergeGuestHint':
    "Enter the guest's real DUPR ID. Their games and statistics will move to the profile with that ID.",
  'player.merged': 'Profiles merged',
  'player.delete': 'Remove from database',
  'player.deleteConfirm':
    "Remove {{name}} from the database? Their Telegram link will be cleared and they can claim again. This player's match history will also be removed.",
  'player.deleted': 'Player removed from the database',
  'player.role': 'Club role',
  'player.roleHint': 'The role is attached to this card’s DUPR ID. Telegram is not required.',
  'player.roleClubAdmin': 'Club administrator',
  'player.roleUpdated': 'Role updated',

  'rating.doubles': 'DUPR doubles',
  'rating.singles': 'DUPR singles',
  'rating.none': 'no rating',
  'rating.noneShort': 'NR',
  'rating.noneHint': 'This player does not have a calculated DUPR rating yet',
  'rating.edit': 'Edit rating',
  'rating.updated': 'Rating updated',
  'rating.updatedAt': 'Updated {{date}}',
  'rating.stale': 'Rating has not been updated for a while',
  'rating.sourceImport': 'from import',
  'rating.sourceModerator': 'set by organiser',
  'rating.sourceSelf': 'set by player',
  'rating.history': 'Rating history',
  'rating.historyEmpty': 'No changes yet',
  'rating.revert': 'Restore this value',
  'rating.range': 'From 2.000 to 8.000',
  'rating.importConflict': 'The latest import has a different value: {{value}}',
  'rating.acceptImport': 'Accept {{value}}',
  'rating.keepCurrent': 'Keep current',

  'settings.title': 'Settings',
  'settings.language': 'Language',
  'settings.languageRu': 'Русский',
  'settings.languageEn': 'English',
  'settings.appearance': 'Appearance',
  'settings.reducedMotion': 'Reduce animations',
  'settings.hideAboutTab': 'Hide “The game” tab',
  'settings.hideAboutTabHint': 'The page will still open from a direct link.',
  'settings.notifications': 'Telegram notifications',
  'settings.notificationsHint':
    'Tournament start and results, check-in confirmation, DUPR link and roster changes',
  'settings.about': 'About',
  'settings.saved': 'Settings saved',
  'settings.version': 'Version',
  'settings.feedback': 'Feedback & bugs',

  'about.title': 'The game',
  'about.lead':
    'Pickleball is a popular sport that mixes tennis, badminton and table tennis. The rules are easy: you can figure them out in an evening and get on court.',
  'about.lead2':
    'You play with a plastic ball full of holes and a solid paddle, on a court the size of a badminton court. The net is like tennis, only a little lower. You can play singles or doubles, indoors or out.',
  'about.courtTitle': 'The court',
  'about.courtCaption':
    'The court is 13.41 × 6.10 m. On each side of the net sits a 2.13 m kitchen: you cannot hit out of the air from there. The serve is from behind the baseline, diagonally into the opponent’s box.',
  'about.courtKitchen': 'Kitchen',
  'about.courtServe': 'Serve',
  'about.courtReceive': 'Return',
  'about.courtNet': 'Net height',
  'about.dimLength': '13.41 m',
  'about.dimWidth': '6.10 m',
  'about.dimKitchen': '2.13 m',
  'about.dimNet': '86 cm',
  'about.equipment': 'A paddle and a plastic ball with holes — that’s all you need.',
  'about.serveTitle': 'The serve',
  'about.serveBody':
    'Underhand, below the waist, diagonally. At least one foot stays down — no jump serves. One attempt.',
  'about.bounceTitle': 'Two bounces',
  'about.bounceBody':
    'The return has to bounce. So does the server’s next shot. From the third shot you may hit out of the air — unless you are standing in the kitchen.',
  'about.kitchenTitle': 'The kitchen',
  'about.kitchenBody':
    'You cannot hit out of the air from the kitchen or its line. If a volley’s momentum carries you in, that’s a fault. You may step in; you may hit from there only after the ball bounces.',
  'about.scoreTitle': 'Scoring',
  'about.scoreBody':
    'Only the serving side scores. Games are usually to 11, win by two: 11–9 is over, 11–10 keeps going. Tournaments sometimes play to 15 or 21.',
  'about.scoreCall':
    'In singles the score is two numbers: yours — theirs. In doubles it is three: yours — theirs — server number (1 or 2).',
  'about.singlesTitle': 'Singles',
  'about.singlesBody':
    'The game starts at 0–0. Serve from the right when your score is even, from the left when it is odd. Lose the rally and the serve goes to your opponent.',
  'about.doublesTitle': 'Doubles',
  'about.doublesBody':
    'The game starts at 0–0–2. Partners switch sides only when they win a point on their serve. Lose the rally and the other team serves.',
  'about.faultsTitle': 'The rally is lost if',
  'about.faultNet': 'the ball hits the net and does not go over',
  'about.faultBounce': 'the two-bounce rule is broken',
  'about.faultBody': 'the ball hits a player or their clothing',
  'about.faultOut': 'the ball lands out',
  'about.faultKitchen': 'you hit out of the air from the kitchen or its line',
  'about.faultReceiver': 'in doubles, the wrong player returns the serve',
  'about.video': 'Watch on YouTube',
  'about.historyTitle': 'A bit of history',
  'about.historyBody':
    'The game appeared in 1965 on Bainbridge Island, near Seattle. Three friends — Joel Pritchard, Bill Bell and Barney McCallum — made something for their kids: paddles, a plastic ball and a lowered badminton net. That was pickleball.',
  'about.clubTitle': 'Here in Russia',
  'about.clubBody':
    'PICKLEBALL Events is growing the game in Moscow: somewhere to show up, people to play with, and a friendly tournament when you want one. Pick your category and go. The rest the game will teach you.',

  'import.title': 'Import players',
  'import.hint':
    'Upload a DUPR export: CSV, JSON, or players.js. Players are matched by DUPR ID, so no duplicates appear.',
  'import.upload': 'Choose file',
  'import.run': 'Import',
  'import.done': 'Import finished',
  'import.report': 'Added {{created}}, updated {{updated}}, conflicts {{conflicts}}',
  'import.conflicts': 'Rating conflicts',
  'import.conflictsHint':
    'These players had their rating set manually and the import has a different value. Choose which one to keep.',
  'import.badFile': 'Could not read the file. Use CSV, JSON, or players.js from a DUPR export.',

  'admin.title': 'Administration',
  'admin.accounts': 'Accounts',
  'admin.moderators': 'Moderators',
  'admin.addModerator': 'Make moderator',
  'admin.removeModerator': 'Remove moderator',
  'admin.auditLog': 'Activity log',
  'admin.bootstrapHint':
    'Administrators, moderators and organizers are listed. Roles are stored on the DUPR ID; PZQZKM and P5ML0M cannot be demoted.',
  'admin.stats': 'Statistics',
  'admin.statsHint':
    'Unique people who opened the Mini App. Today starts at midnight Moscow time; week and month are 7 and 30 days.',
  'admin.statsToday': 'Today',
  'admin.statsWeek': '7 days',
  'admin.statsMonth': '30 days',
  'admin.statsAll': 'All-time via Telegram',
  'admin.statsPlayHint': 'Players in the last 30 days',
  'admin.statsNew30d': 'New accounts, 30 days',
  'admin.statsIdentities': 'Guests · DUPR',
  'admin.statsTournaments30d': 'Tournament players',
  'admin.statsOpenPlay30d': 'Open Play players',
  'admin.statsAvgTournamentDupr': 'Avg tournament DUPR',
  'admin.statsNeverJoined': 'Never joined a game, all-time',
  'admin.statsJoinSource': 'Self · organiser',

  'errors.unauthorized': 'You need to sign in',
  'errors.forbidden': 'Not enough permissions for this action',
  'errors.not_found': 'Not found',
  'errors.validation_failed': 'Please check the form fields',
  'errors.conflict_version': 'The data was changed on another device',
  'errors.duplicate_dupr_id': 'A player with this DUPR ID already exists',
  'errors.dupr_id_already_claimed': 'This DUPR ID is already linked to another account',
  'errors.bootstrap_code_invalid': 'Invalid administrator code',
  'errors.invite_invalid': 'The invite link is invalid or already used',
  'errors.tournament_wrong_status': 'This action is unavailable in the current tournament status',
  'errors.already_in_parallel_tournament': 'Already registered in a parallel tournament',
  'errors.not_all_confirmed': 'Confirm all participants first',
  'errors.not_enough_players': 'This format needs more players',
  'errors.schedule_impossible': 'A schedule cannot be built with these settings',
  'errors.round_not_finished': 'Finish all games in the current round first',
  'errors.match_wrong_status': 'The game is already in a different state',
  'errors.score_required': 'Enter the score for every game',
  'errors.rate_limited': 'Too many requests, please try again',
  'errors.internal': 'Something broke on the server',
  'errors.network': 'No connection to the server',
  'errors.unknown': 'Unknown error',
  'errors.actionFailed': 'The action could not be completed',

  'public.title': 'Scoreboard',
  'public.spectatorHint': 'Live tournament scoreboard. Updates automatically.',

  'training.empty': 'No open games yet',
  'training.emptyHint': 'Create a game and share the link with players',
  'training.organizerHint': 'To create an open game you need the organiser role. Message',
  'training.organizerHintAfter': 'on Telegram — we can help you get access.',
  'training.create': 'Create',
  'training.edit': 'Edit training',
  'training.title': 'Title',
  'training.startsAt': 'Date and time',
  'training.maxPlayers': 'Player limit',
  'training.maxPlayersHint': 'Leave empty for unlimited spots',
  'training.pricePerCourtHour': 'Court price per hour',
  'training.courtBlocks': 'Court rental',
  'training.courtBlocksHint':
    'You can add several blocks: e.g. 1 court × 1 h and 2 courts × 2 h. The total is split among signed-up players.',
  'training.blockCourts': 'Courts',
  'training.blockHours': 'Hours',
  'training.addBlock': 'Add block',
  'training.removeBlock': 'Remove',
  'training.courtHours': '{{hours}} court·h',
  'training.totalCost': 'Rental total: {{amount}}',
  'training.totalCostLabel': 'Rental total',
  'training.suggestedShare': 'Per person: {{amount}}',
  'training.description': 'Description',
  'training.venue': 'Venue',
  'training.created': 'Training created',
  'training.updated': 'Changes saved',
  'training.deleted': 'Training deleted',
  'training.deleteConfirm': 'Delete training "{{title}}"?',
  'training.info': 'Information',
  'training.players': 'Players',
  'training.join': 'Sign up',
  'training.leave': 'Cancel signup',
  'training.joined': 'You are signed up for the training',
  'training.left': 'Signup cancelled',
  'training.needDupr': 'Link your DUPR to sign up',
  'training.appLink': 'Link to training in the bot',
  'training.appLinkCopied': 'Link copied — share it in chat',
  'training.start': 'Start training',
  'training.startHint': 'Everyone signed up is confirmed — ready to start',
  'training.startConfirm': 'Start training for {{count}} players?',
  'training.started': 'Training started',
  'training.runningHint': 'Training is in progress. You can finish anytime.',
  'training.finish': 'Finish training',
  'training.finishShort': 'Finish',
  'training.finishConfirm': 'Finish the training? Amounts can no longer be edited.',
  'training.finished': 'Training finished',
  'training.statusFinished': 'Finished',
  'training.amount': 'Amount due',
  'training.amountEdit': 'Edit amount',
  'training.participantsCount': '{{count}} of {{max}}',
  'training.participantsOpen': '{{count}} signed up',
  'training.amountsReady': 'Amounts calculated — edit with the pencil if needed',
  'training.distribution': 'Distribution',
  'training.distributedOf': '{{distributed}} of {{total}} ₽',
  'training.undistributed': 'Undistributed {{amount}} ₽',
  'training.overdistributed': 'Over by {{amount}} ₽',
  'training.distributionOk': 'Fully distributed',
};
